...
> **Перевимір 7 вер 2026 на 2104 точках за 4 доби: каданс насправді ~5 хвилин.**
> `grid`: медіана 5.1 хв, p99 5.8 хв, максимум 10.9 хв на 969 точок.
> `battery`: медіана 5.1 хв, максимум 15.3 хв на 1135 точок. Розривів понад
> 20 хв — жодного.
>
> Таймстемп точки береться з `collectionTime` самої хмари, тож це каданс
> хмари, а не нашого полінгу: якби хмара віддавала закешоване значення,
> точки збігалися б за таймстемпом і зникали в дедупі `selectNewPoints`.
>
> Оцінка «10–15 хв» вище побудована на **двох** інтервалах — вибірка
> виявилась непредставницькою. `COLLECT_INTERVAL = 300с` при кадансі 5 хв
> означає, що частину оновлень ми бачимо на цикл пізніше; це не помилка,
> але й запасу, який передбачався, немає. Рекомендація для `COLLECT_INTERVAL` у задачі 03: **300с (5 хв)** як
...
> **Оновлено 9 вер 2026: `COLLECT_INTERVAL` = 90с.** Саме «марнотратний, але
> нешкідливий» і виявився правильним компромісом. При рівних періодах (5 хв
> каданс, 5 хв цикл) фаза між ними випадкова, тож у середньому пів циклу —
> ~2.5 хв, у гіршому разі всі 5 — ми чекали на точку, яка вже лежала в хмарі.
> З 90с ця складова падає до ≤90 с, а бакет від цього не росте: дедуп за
> `collectionTime` у `selectNewPoints` відсіює повтори.
>
> Чого це **не** лікує: каданс хмари (~5 хв) і відставання `collectionTime`
> від часу запиту (~6 хв на вимірі 3 вер). Разом стеля затримки лишається
> ~6–11 хв замість колишніх 10–20. Секунди дає лише локальний Modbus.
>
> Ліміти API досі невідомі, тож захист поставлено з двох боків: конфіг не
> приймає інтервал менший за 30с, а кожен збій циклу подвоює паузу до стелі
> в 10 хв (`collector/schedule.js`). Якщо троттлінг таки прийде, він виглядатиме
> як `success:false` з числовим `code` — і його видно в логах Fly як
> «Цикл впав (N поспіль)».

## `device/history` і `measurePoints` — перевірено 9 вер 2026

Обидва знадобились, коли шукали, звідки застосунок Deye знає про зникнення
світла (див. `grafana-alerting.md`, правило 5).

`device/history` з `granularity: 1` віддає **сирі кадри за добу**, а не
агрегати: `dataList[].time` (epoch seconds) плюс `itemList` із запитаними
ключами. Два обмеження, знайдені емпірично:

- `endAt` обовʼязковий навіть при `granularity: 1`, інакше `2101006 endAt
  can't be empty` — попри те, що приклад у репозиторії Deye його для цієї
  гранулярності не показує;
- `measurePoints` не більше **чотирьох** ключів за виклик: пʼять і більше
  дають `2101007 list too long`.

`measurePoints` для SUN-15K повертає **100 ключів, і всі — аналогові**
вимірювання. Немає ані `alarm`, ані `fault`, ані `status`, ані стану реле.

**Наслідок, який варто памʼятати:** причину аварії через OpenAPI дізнатись
неможливо в принципі. Ані окремого ендпойнта аварій (їх немає в офіційному
репозиторії прикладів — лише `account`, `commission`, `device`, `station`,
`strategy`), ані поля стану в кадрі. Усе, що ми можемо, — виводити події з
аналогових величин. Саме тому «інвертор відʼєднався від мережі»
розпізнається за нульовим струмом, а не за кодом помилки.
...
## Знак `BatteryPower` встановлено: мінус — це заряд (4 вер 2026)

Питання, що двічі лишалось відкритим, закрито вимірами на 20 годинах
безперервного збору. Найпряміший доказ — кореляція зі зміною SOC — виявився
недоступним: **SOC не змінився жодного разу**, рівно 98% на всіх 157 точках.
Батарея повна й плаває, тож рухатись їй нікуди.

Спрацював інший, фізично надійніший показник — напруга на клемах:

| Вибірка | Точок | Середня напруга | Середній струм |
|---|---|---|---|
| `power < 0` | 24 | **55.09 V** | −7.31 A |
| `power > 0` | 133 | 54.58 V | +0.93 A |
| різниця | | **+0.51 V** | |

При незмінному SOC напруга спокою стала, отже різниця на клемах — це падіння
на внутрішньому опорі: `V = V_oc ± I·R`. Вища напруга при відʼємній
потужності означає струм, що тече **в** батарею. Під розрядом 7 А напруга
просідала б, а не зростала. Оцінка опору з цих чисел: 0.51 V / 8.2 A ≈ 62 мОм,
що правдоподібно для 48-вольтового блока з кабелями й BMS.

Разом це третій незалежний доказ, і всі троє вказують в один бік:

1. **Баланс потужностей** (3 вер): мережа 742 Вт при навантаженні 239 Вт і
   батареї −405 Вт сходиться лише якщо мінус це заряд; інакше ~900 Вт зникають.
2. **Стрибок напруги** (3 вер): 54.2 → 56.2 V за шість хвилин при −400 Вт.
3. **Статистика за 20 годин** (вище).

**Отже початкове припущення задачі 03 було оберненим.**

### Що з цього змінено

- `helpers.js` отримав `batteryState(power)`: `< 0` → «заряджається»,
  `> 0` → «розряджається», `|power| < 5 Вт` → «у спокої». Мертва зона потрібна,
  бо BMS постійно тримає малий трикл, і без неї підпис блимав би щоп'ять хвилин.
- `/status` знову показує стан — але поруч із числом, яке той пояснює, а не
  окремим рядком.
- **Поле `state` в InfluxDB лишається як є** — `Math.sign(power)`, тобто сирий
  знак без інтерпретації. Перевертати його заднім числом означало б розрив
  семантики посеред 30-денного вікна retention: той, хто побудує графік по
  `state`, побачив би переворот 4 вересня без жодної позначки. Значення
  тлумачить споживач, і тлумачення записане тут.

### Що остаточно підтвердить

Реальний розряд під час відключення: SOC піде вниз, і кореляція стане прямою.
До того моменту висновок спирається на фізику, а не на спостережений цикл.
