...
```bash
npm test        # 327 тестів, бот і колектор разом
```
...
Окремо від них — `data/predeploy-<час знімка>-<версія>.db`, знімок при завершенні процесу,
зберігається 3. **Саме його брати, якщо зламала міграція:** добовий бекап
перезаписується на кожному старті, тож копія до міграції гине від копії після
неї за секунди. Той самий дамп у JSON приходить у чат адміна при першому
виході кожної версії.
...
- **Жодних `<` і `>` у тексті алерту.** Правило колись називалось
  «SOC < 20%» і мовчало: telegram-інтеграція Grafana слала `parse_mode: HTML`.
  З 14 вер 2026 тексти правил ідуть лише через бота, а він екранує HTML, тож
  зараз це не ламає доставку. Але якщо повернути пряму telegram-інтеграцію до
  правил, пастка повернеться разом з нею.
