...
- [ ] `getSubscribers()` не повертає `pending` і `rejected` — покрито тестом
- [ ] FK не дає підписати неіснуючого користувача
- [ ] Каскад: видалення користувача прибирає його підписки
- [ ] `migrate()` на БД з `alert_state` прибирає її й ставить `user_version = 1`
- [ ] Повторний виклик `migrate()` — no-op
- [ ] `replaceSubscriptions()` переводить `approved` → `pending` в одній транзакції
- [ ] Усі таймстемпи парсяться `new Date()` без зсуву — тест на `Z` у кінці
- [ ] Тести на `new Database(':memory:')`, без файлів і моків
