...
Аналіз станом на 2026-08-06. Повний розбір і обґрунтування рішень — у
`docs/plans/`.

> **Стан на 4 вер 2026: усі 24 задачі закриті, система працює.** Колектор пише
> телеметрію кожні 5 хв, бот відповідає, чотири правила алертів адресуються
> персонально підписникам, адмінка на місці. Розділ нижче описує те, що
> зʼясувалося **на початку** роботи, і лишається як історія — не як опис
> сьогоднішнього стану.

## Головне, що з'ясувалося (серпень 2026)
...
### Після фази 5 — зміни на запит власника

Нумерованих задач для них не заводили: це не сплановані роботи, а запити, що
зʼявились уже під час експлуатації. Кожна дописана в ту задачу, чий модуль
змінює, — щоб опис не розходився з кодом.

| Зміна | Де записано |
|---|---|
| Знак `BatteryPower`: відʼємне = заряджається | [03](03-collector.md), `docs/deye-cloud-api.md` |
| Другий вимір `grid` у колекторі | [03](03-collector.md) |
| Правило «немає живлення від мережі» | [12](12-grafana-alert-multi.md), `docs/grafana-alerting.md` §4 |
| Порядок меню: `/status` і `/graph` першими | [10](10-commands-multi.md) |
| Рядок «Мережа: є / немає» у `/status` | [10](10-commands-multi.md) |
| Назви обʼєктів з адмінки, у підписках і алертах | [23](23-admin-web.md) |
| Вигляд адмінки й поділ на Користувачів та Обʼєкти | [23](23-admin-web.md) |
