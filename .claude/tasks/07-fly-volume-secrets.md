...
Волюм `bot_data` створено (`vol_r7yddpleg50pe83r`, fra, 1 GB) — до деплою,
інакше Fly відмовив би. Додано секрет `DEFAULT_INVERTER_ID` зі значенням SN
інвертора: він мусить збігатися з тегом, який пише колектор.

Бот задеплоєний уперше з 6 бер 2026: машина `1854d35f5113d8`, версія 6,
`1 total, 1 passing`.

Health-check поводиться чесно вже на старті:

```
18:05:44  Health check 'servicecheck-00-http-8080' has failed   ← полінгу ще не було
18:05:48  Bot is running! Polling for messages...
18:05:48  Health check 'servicecheck-00-http-8080' is now passing
```

`/status` у Telegram віддає живі дані — критерій задачі 05 закрито заднім
числом. Бот на старті підхопив команду `/help`, яка чекала в черзі Telegram
з 19:46 попереднього вечора.

`ADMIN_CHAT_ID` **не заводив**: `config.js` його експортує, але жоден модуль
не використовує — адмінські команди зʼявляться у фазі 3. Секрет без
споживача лише додає плутанини; завести його разом із задачею 11.
